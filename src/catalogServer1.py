...
@catalogServer1.route("/search/<string:topic>/",methods=['GET'])
def search(topic):
	'''This method searches the catalog for books belonging to the specified topic, and returns them in a JSON object'''
	d,d2 = {},{}
	with open('catalog1.csv') as csvfile: # Read catalog file and check for matches with the specified topic
...
@catalogServer1.route("/lookup/<string:item_number>/",methods=['GET'])
def lookup(item_number):
	...
	d = {}
	with open('catalog1.csv') as csvfile: # Read catalog file and check for matches with the specified item number
		catalogreader = csv.DictReader(csvfile)
		for row in catalogreader:
...
@catalogServer1.route("/buy/",methods=['POST'])
def buy():
	...
	try:
		with open('catalog1.csv','r') as csvfile: # Read catalog file, check for match with the specified item number and check its stock
			...
			for row in catalogreader:
				...
				if row['ItemNumber'] == item_number and int(row['Stock']) > 0: # If match and in stock, decrement stock value
					...
					urlCatalog2 = 'http://' + catalogIP2 + ':' + catalogPort2 + '/update/' + row['ItemNumber'] + ':' + 'buy'
					if flags['catalog2'] == 1:
						r_catalog2 = requests.put(urlCatalog2, timeout = 5)
					else:
						print('catalogServer2 not updated on buy because it is down')
		...
		with open('catalog1.csv', 'w') as csvfile: # Write changes to catalog file
...
def checkAndRestock():
	...
	while True:
		time.sleep(30)
		url = 'http://' + catalogIP1 + ':' + catalogPort1 + '/restock/'
		r = requests.post(url)

@catalogServer1.route("/restock/",methods=['POST'])
def restock():
	''' This method checks the catalog file, and if any item is found to be out of stock, it restocks it.'''
	global flags
	change = False
	with open('catalog1.csv','r') as csvfile: # Read catalog file
		catalogreader = csv.DictReader(csvfile)
		for row in catalogreader:
	...
	if change: # Execute only if a change needs to be made
		...
		try:
			with open('catalog1.csv','r') as csvfile:
				catalogreader = csv.DictReader(csvfile)
				for row in catalogreader:
					new_row = dict(row)
					if int(row['Stock']) <= 0: # If found item out of stock, restock it.
						...
						if flags['catalog2'] == 1:
							...
							print('catalogServer2 not updated on restock because it is down')
						urlFrontEnd = 'http://' + frontEndIP + ':' + frontEndPort + '/invalidate/' + row['ItemNumber']
					new_rows.append(new_row)
			fieldnames = ['ItemNumber', 'Title','Topic','Cost','Stock']
			with open('catalog1.csv', 'w') as csvfile: # Write changes to catalog file
...
if __name__ == "__main__":
	try:
		...
		print('unable to sync because catalogServer2 is not up')
	with open('config.json') as json_file:
		data = json.load(json_file)
		portnum = data['CatalogServer1'].split(":")[1]
	serverThread = threading.Thread(target = catalogServer1.run,kwargs = {'host' : '0.0.0.0','port' : portnum,'threaded':True}) # This is the server thread
